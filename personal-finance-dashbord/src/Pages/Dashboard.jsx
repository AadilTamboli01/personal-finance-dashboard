...
const Dashboard = ({ balance, income, expense, transactions }) => {
  ...
  return (
    <div className="p-6 min-h-screen bg-gradient-to-br from-slate-100 via-gray-100 to-slate-200 dark:from-gray-900 dark:via-gray-950 dark:to-gray-900 transition-colors duration-300">
      ...
      <div className="grid md:grid-cols-3 gap-6 mt-6">
        {[
          ...
          { title: "Total Expense", value: `₹ ${totalExpense}`, color: "text-red-500" },
        ].map((item, index) => (
          <motion.div
            key={index}
            ...
            className="bg-white/60 dark:bg-gray-800/60 backdrop-blur-xl p-6 rounded-2xl shadow-md hover:shadow-2xl transition"
          >
            <p className="text-gray-500 dark:text-gray-400 text-sm">{item.title}</p>
            <h2 className={`text-xl font-bold mt-2 dark:text-white ${item.color || ""}`}>
              {item.value}
            </h2>
          </motion.div>
        ))}
      ...
      <motion.div
        ...
        className="bg-white/70 dark:bg-gray-800/70 backdrop-blur-xl mt-6 p-6 rounded-2xl shadow-lg transition-colors duration-300"
      >
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {months.map((month, index) => (
            <motion.div
              ...
              whileHover={{ scale: 1.05 }}
              className="bg-gradient-to-r from-gray-50 to-gray-100 dark:from-gray-700 dark:to-gray-600 p-4 rounded-xl text-center shadow-sm hover:shadow-md"
            >
              <p className="text-gray-500 dark:text-gray-300 text-sm">{month}</p>
              <p className="font-semibold text-lg dark:text-white">₹ {monthlyData[month]}</p>
            </motion.div>
          ))}
        </div>
      </motion.div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mt-6">
        <motion.div
          ...
          className="bg-white/70 dark:bg-gray-800/70 backdrop-blur-xl p-5 rounded-2xl shadow-lg"
        >
